...
import 'package:jendela_dbp/components/tv/youtube_player_card.dart';
...
class TvScreen extends StatefulWidget {
  const TvScreen({Key? key, required this.tv}) : super(key: key);

  final Tv tv;

  @override
  // ignore: library_private_types_in_public_api
  _TvScreen createState() => _TvScreen();
}

class _TvScreen extends State<TvScreen> {
  TvBloc relatedVideoBloc = TvBloc();
  MediaBloc featuredMediaBloc = MediaBloc();
  late AuthCubit _authCubit;
  // YoutubePlayerController _playerController = YoutubePlayerController(
  //   initialVideoId: 'CHfycnQZQGo', //Add videoID.
  ...
  @override
  void initState() {
    relatedVideoBloc.add(TvFetch());
    featuredMediaBloc.add(MediaFetchById(mediaId: widget.tv.featuredMedia));
    super.initState();
  }

  @override
  void dispose() {
    relatedVideoBloc.close();
  ...
  Widget build(BuildContext context) {
    ...
    Uri uri = Uri.parse(
        widget.tv.guid?['rendered'] ?? "https://{GlobalVar.BaseURLDomain}");
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.tv.title!['rendered'],
                  ...
                  if (state is MediaLoaded) {
                    return Column(
                      children: [
                        SizedBox(
                          child: Align(
                            ...
                            child: FittedBox(
                                ...
                                //     progressIndicatorColor: colors.bgPrimaryColor,
                                //   ),
                        ...
                        Text(
                          parse(widget.tv.content!['rendered']).body!.text,
                          style: const TextStyle(
                              color: Colors.black,
                              fontSize: 16.0,
                            ...
                            children: [
                              ...
                              TextButton(
                                ...
                                child: Text(
                                  'Lihat Semua',
                                  style:
                                      TextStyle(color: DbpColor().jendelaGray),
                                ),
                              )
                            ],
                          ),
                        ...
                        SizedBox(
                          ...
                          child: BlocBuilder<TvBloc, TvState>(
                            bloc: relatedVideoBloc,
                            builder: (context, data) {
                              if (data is TvError) {
                                return Center(child: Text(data.toString()));
                              }
                              if (data is TvLoaded) {
                                List<Widget> listWidget = data.tv!.map<Widget>(
                                  (item) {
                                    return TvCard(
                                      tv: item,
                                      viewCount: 212,
